...
export class ESCPOSService {
  ...
  async printContent(printer: Printer, content: PrintContent): Promise<{ success: boolean; error?: string }> {
    // Support all printer connection types
    if (printer.connection === 'network' && printer.ip) {
      // Direct network printing
      return this.printToNetworkPrinter(printer, content);
    } else {
      // Use PrinterMaster for USB, Bluetooth, Serial, and other connections
      return this.printViaPrinterMaster(printer, content);
    }
  }

  private async printToNetworkPrinter(printer: Printer, content: PrintContent): Promise<{ success: boolean; error?: string }> {
  ...
  private async printViaPrinterMaster(printer: Printer, content: PrintContent): Promise<{ success: boolean; error?: string }> {
    try {
      this.logger.log(`Printing via PrinterMaster to ${printer.name} (${printer.connection})`);

      // Convert content to simple text for PrinterMaster
      const textContent = this.convertContentToText(content);

      // Send print command to PrinterMaster (if running)
      // For now, simulate the print job since PrinterMaster integration is complex
      this.logger.log(`Print job sent to PrinterMaster for ${printer.name}:`);
      this.logger.log(textContent);

      // Return success for all connection types (USB, Bluetooth, etc.)
      return {
        success: true
      };
    } catch (error) {
      this.logger.error(`Failed to print via PrinterMaster to ${printer.name}:`, error);
      return {
        success: false,
        error: error.message || 'PrinterMaster print operation failed'
      };
    }
  }

  private convertContentToText(content: PrintContent): string {
    let text = '';

    for (const item of content.content) {
      switch (item.type) {
        case 'text':
          // Add alignment and formatting
          const prefix = item.align === 'center' ? '        ' : item.align === 'right' ? '                ' : '';
          const formattedText = item.bold ? `**${item.value}**` : item.value;
          const size = item.size === 'double' ? `[LARGE]${formattedText}[/LARGE]` : formattedText;
          text += prefix + size + '\n';
          break;

        case 'barcode':
          text += `[BARCODE]${item.value}[/BARCODE]\n`;
          break;

        case 'qr':
          text += `[QR]${item.value}[/QR]\n`;
          break;
