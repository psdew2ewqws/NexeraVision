...
import { SyncProgressEvent, BatchSyncProgressEvent } from '../types/sync.types';
...
export class SyncProgressGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleSyncSubscription(
    ...
    try {
      ...
      const syncStatus = await this.platformSyncService.getSyncStatus(data.syncId, userInfo.companyId);
      if (!syncStatus) {
  ...
  async handleMultiSyncSubscription(
    ...
    try {
      ...
      const multiSyncStatus = await this.platformSyncService.getBatchSyncStatus(data.multiSyncId, userInfo.companyId);
      if (!multiSyncStatus) {
  ...
  // ================================================
  // SYNC PROGRESS EMISSION METHODS
  // ================================================

  /**
   * Emit sync started event
   */
  emitSyncStarted(companyId: string, data: any) {
    this.emitToCompany(companyId, 'sync-started', {
      type: 'sync-started',
      data,
      timestamp: new Date()
    });
   ...
   */
  emitSyncProgress(companyId: string, progress: SyncProgressEvent) {
    this.emitToCompany(companyId, 'sync-progress', {
      type: 'sync-progress',
      data: progress,
      timestamp: new Date()
    });
   ...
   */
  emitSyncCompleted(companyId: string, data: any) {
    this.emitToCompany(companyId, 'sync-completed', {
      type: 'sync-completed',
      data,
      timestamp: new Date()
    });
   ...
   */
  emitSyncFailed(companyId: string, data: any) {
    this.emitToCompany(companyId, 'sync-failed', {
      type: 'sync-failed',
      data,
      timestamp: new Date()
    });
   ...
   */
  emitSyncCancelled(companyId: string, data: any) {
    this.emitToCompany(companyId, 'sync-cancelled', {
      type: 'sync-cancelled',
      data,
      timestamp: new Date()
    });
  ...
  /**
   * Emit batch sync started event
   */
  emitBatchSyncStarted(companyId: string, data: any) {
    this.emitToCompany(companyId, 'batch-sync-started', {
      type: 'batch-sync-started',
      data,
      timestamp: new Date()
    });
  }
  ...
  /**
   * Emit batch sync progress update
   */
  emitBatchSyncProgress(companyId: string, progress: BatchSyncProgressEvent) {
    this.emitToCompany(companyId, 'batch-sync-progress', {
      type: 'batch-sync-progress',
      data: progress,
      timestamp: new Date()
    });
  }
   ...
   */
  private emitToCompany(companyId: string, event: string, data: any) {
