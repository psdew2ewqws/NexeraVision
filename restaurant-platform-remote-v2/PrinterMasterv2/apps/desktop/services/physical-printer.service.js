...
class PhysicalPrinterService {
  constructor() {
    this.printers = new Map(); // Cache of initialized printers
    this.isInitialized = false;
    this.supportedTypes = ['thermal', 'regular', 'pdf', 'kitchen'];
  }
  ...
  async initialize() {
    ...
    try {
      ...
      console.warn('⚠️ [PHYSICAL-PRINTER] Thermal printer library initialization warning:', error.message);
   ...
   * @param {Object} testData - Test page data
   * @returns {Promise<Object>} Print result
   */
  async printTestPage(printerConfig, testData = {}) {
  ...
  async printToThermalPrinter(printerConfig, content) {
    try {
      ...
      let printerInterface = this.determinePrinterInterface(printerConfig);
   ...
   */
  async printToThermalPrinterFallback(printerConfig, content) {
   ...
   */
  async printToRegularPrinter(printerConfig, content) {
   ...
   */
  async printToKitchenPrinter(printerConfig, content) {
   ...
   */
  async printToPDF(printerConfig, content) {
  ...
  async testPrinterConnection(printer) {
    try {
      ...
      console.warn('⚠️ [CONNECTION-TEST] Connection test failed:', error.message);
      return false;
    }
  }
   ...
   */
  generateTestPageContent(printerConfig, testData) {
    return {
   ...
   */
  detectThermalPrinterType(printerConfig) {
   ...
   */
  determinePrinterInterface(printerConfig) {
   ...
   */
  findUSBInterface(printerConfig) {
   ...
   */
  findSerialInterface(printerConfig) {
    const platform = os.platform();
   ...
   */
  findBluetoothInterface(printerConfig) {
   ...
   * Generate raw ESC/POS commands
   */
  generateESCPOSCommands(printerConfig, content) {
    const ESC = '\x1b';
    const GS = '\x1d';
   ...
   */
  async sendRawToSystemPrinter(printerConfig, rawData) {
   ...
   */
  async sendRawToNetworkPrinter(printerConfig, rawData) {
   ...
   */
  formatContentForRegularPrinter(content) {
    let text = '';
   ...
   */
  async sendTextToSystemPrinter(printerConfig, textContent) {
   ...
   */
  async generatePDFForPrinting(content) {
   ...
   */
  async sendPDFToSystemPrinter(printerConfig, pdfPath) {
   ...
   */
  async getPrinterStatus(printerConfig) {
    await this.initialize();
   ...
   */
  async processPrintJob(printerConfig, jobData) {
    await this.initialize();
   ...
   * Print receipt
   */
  async printReceipt(printerConfig, jobData) {
   ...
   * Print kitchen order
   */
  async printKitchenOrder(printerConfig, jobData) {
   ...
   * Print label
   */
  async printLabel(printerConfig, jobData) {
  ...
  generateReceiptContent(jobData) {
  ...
  generateKitchenOrderContent(jobData) {
...
module.exports = PhysicalPrinterService;