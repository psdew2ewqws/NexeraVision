...
export class MultiPlatformSyncService {
  ...
  async getMultiSyncStatus(multiSyncId: string): Promise<MultiPlatformSyncStatus> {
    ...
    let cachedStatus = await this.cacheService.get(cacheKey);
    if (cachedStatus) {
      return cachedStatus;
    }
    ...
    return {
      ...
      overallProgress: {
        completedPlatforms: dbMultiSync.completedPlatforms,
        totalPlatforms: dbMultiSync.totalPlatforms,
        percentage: (dbMultiSync.completedPlatforms / dbMultiSync.totalPlatforms) * 100
      },
      platformStatuses: dbMultiSync.individualSyncs.map(sync => ({
        platform: sync.platform as DeliveryPlatform,
        syncId: sync.syncId,
        status: sync.status as SyncStatus,
        progress: sync.progress || {},
        errors: sync.errors || []
      })),
      totalItemsSynced: dbMultiSync.totalItemsSynced,
      totalErrors: dbMultiSync.totalErrors,
  ...
  private async recordMultiSyncCompletion(
    ...
    await this.prisma.multiPlatformSyncHistory.update({
      ...
      data: {
        ...
        completedAt: multiSyncJob.completedAt,
        completedPlatforms: status.overallProgress.completedPlatforms,
        totalItemsSynced: status.totalItemsSynced,
        totalErrors: status.totalErrors,
        errorMessage,
        syncDurationMs: multiSyncJob.completedAt
          ? multiSyncJob.completedAt.getTime() - multiSyncJob.startedAt.getTime()
          : 0
      }
    });
  }
