...
const BranchItem = memo<{
  ...
  language: 'en' | 'ar';
}>(({ branch, isSelected, onToggle, language }) => (
  <button
    type="button"
...
// API fetch function
const fetchBranches = async (authToken: string, signal?: AbortSignal): Promise<Branch[]> => {
...
export const BranchSelector = memo<BranchSelectorProps>(({
  selectedBranchIds,
  onBranchChange,
  ...
  // React Query for branches with automatic caching and deduplication
  const {
    data: branches = [],
    isLoading: loading,
    error
    ...
    queryFn: ({ signal }) => fetchBranches(authToken!, signal),
    enabled: !!userId && !!authToken,
    staleTime: 5 * 60 * 1000, // 5 minutes
    cacheTime: 10 * 60 * 1000, // 10 minutes
  });

  // Memoized active branches
  ...
  const allSelected = useMemo(
    () => activeBranches.length > 0 && activeBranches.every(branch => selectedBranchIds.includes(branch.id)),
    [activeBranches, selectedBranchIds]
  );
  ...
  }, [disabled, selectedBranchIds, onBranchChange]);

  // Handle select all
  const handleSelectAll = useCallback(() => {
    if (disabled) return;

    const allBranchIds = activeBranches.map(branch => branch.id);
    onBranchChange(allSelected ? [] : allBranchIds);
  }, [disabled, activeBranches, allSelected, onBranchChange]);
  ...
  }, [selectedBranchIds, branches, activeBranches, language, placeholder]);
        ...
        <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
          {loading ? (
            ...
            <>
              ...
              {/* Individual Branch Options - Memoized */}
  ...
  prevProps.selectedBranchIds === nextProps.selectedBranchIds &&
